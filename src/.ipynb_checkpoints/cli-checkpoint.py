...
Примеры:
  ...
  python -m src.cli stats
    ...
    ingest_parser.add_argument(
        ...
        help="Text-only режим: не извлекать и не анализировать изображения"
    )
    ...
    pipeline = MultimodalRAGPipeline(
        ...
        process_images=not getattr(args, "no_images", False),
    )

    if args.command == "ingest":
        print(f"\n[DOC] Индексация: {args.pdf_path}\n")
        stats = pipeline.ingest(args.pdf_path)
        print(f"\n[STATS] Результат: {stats}")

    elif args.command == "query":
        print(f"\n[Q] Вопрос: {args.question}\n")
        result = pipeline.query(args.question)

        print(f"[AI] Ответ ({result.get('model', 'unknown')}):\n")
        print(result["answer"])

        if result.get("sources"):
            print(f"\n[SRC] Источники ({len(result['sources'])}):")
