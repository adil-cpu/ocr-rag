"""Generate notebook for дифференциальные_уравнения.pdf (277 pages).
v4: uses max_images parameter in ImageExtractor to prevent OOM.
"""
import json, hashlib
...
cells.append(md("""# Мультимодальный RAG: Дифференциальные уравнения (277 стр.)

**Документ:** дифференциальные_уравнения.pdf (277 страниц, ~15 МБ)  
**Цель:** Нагрузочное тестирование пайплайна  
**Оптимизация:** Лимит изображений (max_images=20), очистка памяти между этапами"""))
...
PDF_PATH = 'data/input_pdfs/дифференциальные_уравнения.pdf'
...
ax.set_title('Распределение типов блоков (277 стр.)')
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
plt.tight_layout()
os.makedirs('report/img', exist_ok=True)
plt.savefig('report/img/diff_block_types.png', dpi=150, bbox_inches='tight')
plt.show()"""))
...
> **max_images=20** — извлекаем максимум 20 изображений, остальные пропускаются.  
> Это экономит ~2 ГБ RAM на большом документе."""))
...
# max_images=20 — ранний выход, не загружаем все 277 страниц
...
chart_only_images = [img for img, cls in chart_images][:5]  # макс 5
chart_results = []
...
if chart_results and chart_only_images:
    for idx in range(min(len(chart_results), 4)):
        ...
        plt.tight_layout()
        plt.savefig(f'report/img/diff_chart_{idx+1}.png', dpi=150, bbox_inches='tight')
        plt.show()
else:
    print('Нет графиков.')
...
found = False
for c in chunks:
    if 'график' in c.text.lower() or 'диаграмма' in c.text.lower():
        print('=== Чанк с транскрипцией ===')
        print(c.text[:500])
        found = True
        break
if not found:
...
ax.spines['right'].set_visible(False)
plt.tight_layout()
plt.savefig('report/img/diff_chunk_sizes.png', dpi=150, bbox_inches='tight')
plt.show()"""))
...
if embeddings.shape[0] > 5:
    ...
    ax.spines['right'].set_visible(False)
    plt.tight_layout()
    plt.savefig('report/img/diff_tsne.png', dpi=150, bbox_inches='tight')
    plt.show()"""))
...
cells.append(code("q1 = ask('О чём этот документ?')"))
cells.append(code("q2 = ask('Какие методы решения дифференциальных уравнений описаны?')"))
cells.append(code("q3 = ask('Какие данные представлены на графиках и диаграммах?')"))
cells.append(code("q4 = ask('Объясни теорему Коши для дифференциальных уравнений.')"))
...
for i, cell in enumerate(cells):
    cell["id"] = hashlib.md5(f"diff_v4_{i}".encode()).hexdigest()[:8]
...
with open("notebooks/diff_equations_result.ipynb", "w", encoding="utf-8") as f:
